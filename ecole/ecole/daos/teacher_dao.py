        ...
        async with session.begin():
            ...
            if id_person:
                res_teacher = await session.execute(
                    text(
                        "INSERT IGNORE INTO teacher (hiring_date, id_person) "
                        "VALUES (:hiring_date, :id_person)"
                    ),
                    {"hiring_date": hiring_date, "id_person": id_person},
                )
...
async def teacher_read(
    ...
    """)
    async with connection() as session:
        return await session.scalar(query, {"id_teacher": id_teacher})
...
async def teacher_add_course(
    ...
    async with connection() as session:
        async with session.begin():
            res = await session.execute(
                text("UPDATE course SET id_teacher = :id_teacher WHERE id_course = :course_id"),
                {"id_teacher": id_teacher, "course_id": course_id},
            )
    ...
    async with connection() as session:
        async with session.begin():
            ...
            await session.execute(
                text("UPDATE teacher SET hiring_date = :hiring_date WHERE id_teacher = :id_teacher"),
                {"hiring_date": hiring_date, "id_teacher": id_teacher},
            )
            return res.rowcount > 0
    ...
    async with connection() as session:
        async with session.begin():
            ...
            if record:
                ...
                await session.execute(
                    text("UPDATE course SET id_teacher = NULL WHERE id_teacher = :id_teacher"),
                    {"id_teacher": id_teacher},
                )
                # 2. Supprimer la fiche enseignant, personne et adresse si présente
                await session.execute(
                    text("DELETE FROM teacher WHERE id_teacher = :id_teacher"),
                    {"id_teacher": id_teacher},
                )
                await session.execute(
